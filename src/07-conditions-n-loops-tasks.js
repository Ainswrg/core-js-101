...
function findFirstSingleChar(str) {
  return str.split('').filter((el) => (str.indexOf(el) === str.lastIndexOf(el) ? el : null))[0] || null;
}
...
function isCreditCardNumber(ccn) {
  const nString = String(ccn);
  const nDigits = nString.length;
  let sum = 0;
  const parity = (nDigits - 2) % 2;
  for (let i = 0; i < nDigits; i += 1) {
    let digit = parseInt(nString[i], 10);
    if (i % 2 === parity) {
      digit *= 2;
    }
    if (digit > 9) {
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < minLength; i += 1) {
    ...
    for (let k = 1; k < paths.length; k += 1) {
      if (paths[k][i] !== currentPath) {
        return commonPath;
      }
    }
...
function evaluateTicTacToePosition(position) {
  ...
  if (winnerRow) {
    return position[winnerRow][0];
  }
  if (winnerColumn) {
    return position[0][winnerColumn];
  }
  if (winnerDiagonal) {
    return position[0][winnerDiagonal];
  }
  return undefined;
}
